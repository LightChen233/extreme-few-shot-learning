...
DOMAIN_PARAMS = {
    # ===== 机制边界 / 工艺窗口 =====
    # 小样本下只做极小幅度微调：保留现有框架，只轻调温度中心与局部尺度
    # 结合误差：470@12 仍是高误差热点，460~470℃附近应允许更明显强化
    'critical_temp_regime_shift': 465.0,
    'critical_time_regime_shift': 12.0,
    'long_time_threshold': 24.0,

    # 轻微上调优区中心，更贴近 470@12 外推热点
...
class FeatureAgent:
    ...
    def physics_baseline(self, temp, time):
        ...
        自检思路：
        - 440@1 相对 420@1：baseline 不应更低太多，应呈桥接增强
        - 440@24 相对 420@24：baseline 不应走向明显软化
        - 470@12 相对 480@12：应允许接近甚至形成局部高值，避免中温高时效被压低
        """
        p = DOMAIN_PARAMS
        temp = float(temp)
    ...
    def engineer_features(self, df):
        ...
        # 极小新增：仅补 1 个“470附近×12h时间窗”的局部峰值协同特征
        # 目的：帮助下游模型识别 470@12 的强度/延性同步抬升，而不大幅扩维
        cols['midhigh_temp_time_synergy'] = (
            np.exp(-((temp - 468.0) / 10.0) ** 2) *
            np.exp(-((log_time - np.log1p(12.0)) / 0.45) ** 2)
        )
