...
DOMAIN_PARAMS = {
    ...
    # 7xxx / 7499铝合金热处理动力学相关经验参数
    # 以下参数为材料机理启发的经验常数，用于构造物理合理特征
    ...
    # 峰值窗口宽度
    'peak_temp_width_C': 16.0,
    'peak_log_time_width': 0.65,
    ...
    # 经验缩放参数
    'softening_temp_scale_C': 22.0,
    'time_saturation_h': 6.0,
    'temp_activation_width_C': 10.0,
    'arrhenius_reference_temp_C': 130.0,

    # 时间-温度等效参数（经验型）
    'hj_constant': 20.0,  # Hollomon-Jaffe / Larson-Miller 风格常数
    'equiv_temp_scale_C': 28.0,

    # 小样本稳健缩放
    'temp_center_scale_C': 30.0,
    'log_time_center_scale': 1.0,
}
...
class FeatureAgent:
    ...
    def engineer_features(self, df):
        ...
        # ==================================================
        # 4. 时间-温度等效老化特征
        # 用于捕捉高温短时 ≈ 低温长时 的工艺补偿关系
        # ==================================================
        hj_const = self.params['hj_constant']
        X['temp_log_time_kelvin'] = temp_k * (hj_const + log_time)
        X['equivalent_aging_index'] = clipped_temp * log_time
        X['equivalent_aging_index_k'] = temp_k * log_time

        # 扩散加速下的等效时间
        X['diffusion_weighted_time'] = clipped_time * np.exp((clipped_temp - self.peak_temp) / self.params['equiv_temp_scale_C'])
        ...
        # 8. 动态组织演化推理特征
        # ==================================================
        # 8.1 析出强化驱动力：在峰值时效附近最强，低温需时间积累，高温后期衰减
        ...
        precipitation_drive[low_idx] = (
            0.40 * log_time[low_idx]
            + 0.005 * (clipped_temp[low_idx] - self.min_temp) * log_time[low_idx]
            + 0.20 * self._safe_log(1e6 * thermal_dose[low_idx] + 1.0)
        )

        precipitation_drive[peak_idx] = (
            0.95 * log_time[peak_idx]
            + 0.012 * (clipped_temp[peak_idx] - self.low_temp_threshold) * log_time[peak_idx]
            + 0.90 * peak_window[peak_idx]
            - 0.030 * np.maximum(clipped_time[peak_idx] - self.medium_time_threshold, 0.0)
        )

        precipitation_drive[high_idx] = (
            0.75 * np.minimum(log_time[high_idx], np.log1p(self.medium_time_threshold))
            + 0.15
            - 0.20 * np.maximum(log_time[high_idx] - np.log1p(self.short_time_threshold), 0.0)
        )

        X['precipitation_drive'] = precipitation_drive

        # 8.2 恢复/粗化/溶解驱动力：高温长时下增强，通常削弱强度并恢复塑性
        recovery_drive = (
        ...
        softening_drive = (
            ...
            + 0.025 * np.maximum(clipped_time - self.medium_time_threshold, 0.0) * high_temp_mask
        )
        ...
        peak_temp_activation = self._gaussian_window(clipped_temp, self.peak_temp, self.peak_temp_width)
        ...
        # ==================================================
        # 12. 强度-塑性竞争特征
        # 强化相细化/增多提升强度，但恢复/粗化往往提升延性
        # ==================================================
        ductility_recovery = (
            0.55 * softening_drive
            + 0.18 * high_temp_mask
            + 0.18 * long_time_mask
            - 0.22 * precipitation_drive
        )
        ...
        # ==================================================
        # 15. 多输出共享状态特征
        # tensile 和 yield 通常强相关；strain 与其存在竞争关系
        # ==================================================
        ...
        # ==================================================
        # 16. 小样本稳健压缩特征
        # 避免高次项过度放大，帮助模型稳定学习非线性
        # ==================================================
        temp_scale = self.params['temp_center_scale_C']
        log_time_scale = self.params['log_time_center_scale']

        X['compressed_temp'] = np.tanh((clipped_temp - self.peak_temp) / temp_scale)
        X['compressed_log_time'] = np.tanh((log_time - peak_log_time) / log_time_scale)
        ...
        # 更直接表达“强化-软化-峰值”三方平衡
        # ==================================================
        X['peak_strength_balance'] = peak_window * net_strengthening
        X['softening_vs_precipitation'] = self._safe_divide(softening_drive + 1e-6, np.abs(precipitation_drive) + 1e-6)
        X['aging_efficiency'] = self._safe_divide(net_strengthening, log_time + 1.0)
        X['high_temp_overaging_risk'] = high_temp_mask * overaging_tendency * (1.0 + log_time)
        X['peak_zone_strength_reserve'] = peak_temp_mask * peak_window * (1.0 - overaging_tendency)
        X['ductility_gain_risk_balance'] = ductility_recovery - 0.5 * softening_drive + 0.2 * overaging_tendency
